...
class Utility 
{
	...
	$module_name = Yii::$app->db->createCommand($sqlmodule_name)->queryOne();
		if(!empty($module_name)){
			if($type=="logo"){
				if(file_exists(Yii::getAlias('@basePath').'/merchant_images/merchantimages/'.$module_name['logo'])){
...
public static function rewards_details($id,$type){  
 ...
 public static function tablereservations_status($status){ 
	...
	public static function getOrderSequence($merchant,$orderType = 'DI') 
	{
		return sprintf('%02d',$merchant).$orderType.(new self)->getOrderAlphabetSequence($merchant);
	}
...
if(!empty($id)){
    ...
    return $result;
	 ...
	 public static function sendNewFCM($id,$title,$message,$imageurl=false,$type=null,$encykey=null,$orderid=null,$notificationdet = [],$extraNotificationData = []) {
...
'12'=>'12 PM','13'=>'1 PM','14'=>'2 PM','15'=>'3 PM','16'=>'4 PM','17'=>'5 PM','18'=>'6 PM','19'=>'7 PM','20'=>'8 PM','21'=>'9 PM','22'=>'10 PM','23'=>'11 PM'
	 ...
	 * @return false|string
	 */
	public static function lastDesiredDate(int $days)
	{
	...
	{
	  // https://stackoverflow.com/questions/14278603/php-range-from-a-to-zz	
	  $columns = array();
	...
	public function getAlphabetSequences(){
	    $alphabetArray = $this->createColumnsArray('ZZ');
		
		$alphabetSequence = [];

		foreach ($alphabetArray as $alphabetPair) {
			if(strlen($alphabetPair) > 1) {
				$alphabetSequence[] = $alphabetPair;
			}
		}
	...
	public  function  getOrderAlphabetSequence($merchant)
	{
		$sqlPreviousOrder = 'select order_id from orders where merchant_id = \''.$merchant.'\' order by ID desc limit 1';
		$prevOrder = Yii::$app->db->createCommand($sqlPreviousOrder)->queryOne();
		
		$alphabetSequences = $this->getAlphabetSequences();
		
		$sequence = 'AA0001'; 

		if (!empty($prevOrder)) {
			$orderPrevAlphabetSeq = substr($prevOrder['order_id'],-6,2);
			$orderPrevOrderSeq = substr($prevOrder['order_id'],-4);
			if(!is_numeric($orderPrevAlphabetSeq)){
				$lastOrderAlphabetIndex = array_search($orderPrevAlphabetSeq,$alphabetSequences);

				if (($orderPrevOrderSeq+1) == 10000) {
					$sequence = $alphabetSequences[$lastOrderAlphabetIndex+1].'0001';
				} else {
					$sequence = $orderPrevAlphabetSeq.sprintf('%04d',($orderPrevOrderSeq+1));
				}
			} 
		}
