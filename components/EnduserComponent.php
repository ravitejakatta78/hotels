...
class EnduserComponent extends Component {
	...
	public function updation($val)
	{
		if(!empty($val['name']) && !empty($val['mobile'])){
						...
						$sqlrow = "SELECT * FROM users WHERE email = '".$userarray['email']."' and ID <> '".$val['header_user_id']."' and email is not null and email <> ''";
						$row = Yii::$app->db->createCommand($sqlrow)->queryOne();
					if(empty($row['ID'])){
	...
	public function merchantbyid($val)
	{
		if(!empty($val['merchantid'])){
					...
					$modelWhishlist = UserWhislist::find()->where(['user_id' => $val['header_user_id'],'merchant_id' => $merchantid,'status' => 1])->One();
					 ...
					 if(!empty($merchantsdata)){
							...
							$merchants['isWhishListed'] = !empty($modelWhishlist) ? true : false;
                            
							$feedbackrating = $this->getMerchantRating($merchantsdata['ID']);
                            
							$merchants['rating'] = $feedbackrating;
							...
							if(!empty($enckey)){
								...
								if($tabel_Det['current_order_id'] != 0 || $tabel_Det['current_order_id'] != null )
								{
								    ...
								    if($currentOrder['user_id'] != $val['header_user_id'] ){
								        $payload = array("status"=>'0', 'tableId' => $tableid,"text"=>"Table is already occupied");
									    return $payload;
									    exit;
								    }    
								}
...
select foodtype,case when foodtype = \'0\' then \'All\'  else fc.food_category end as food_category
			...
			if(!empty($enckey)){
                ...
                if($tableid == 'Parcels') {
					...
					if($tabledetails['status'] != '1') {
						$payload = ['status' => '0', 'text' => 'Table is not available to take orders'];
						return $payload;
						exit;
					}
                }
				...
				if((@$tabledetails['current_order_id'] != 0 || @$tabledetails['current_order_id'] != null) && $merchantdetails['table_occupy_status'] == 1 && $tableid != 'Parcels' )	{
				    ...
				    if( ($currentOrder['user_id'] != $val['header_user_id']) && $qrType != '2'  ){
				        $payload = array("status"=>'0', 'tableId' => $tableid,"text"=>"Table is already occupied");
					    return $payload;
					    exit;
				    }
				...
				if(!empty($merchantdetails)){
					...
					if($latlngdistance > $merchantdetails['scan_range'] && $qrType == '1'){
                        $payload = array("status"=>'0',"text"=>"Restaurent or  theater distance is too long");
                        return $payload;
                        exit;
                    }
					...
					if(!empty($tabledetails)){
                        ...
                        foreach($merchantproductsarray as $merchantproduct){
                            if($merchantproduct['section_item_sale_price'] > 0){
                                ...
                                $singleproducts['availabilty'] = $merchantproduct['availabilty'];
								$singleproducts['upselling'] = $merchantproduct['upselling'];
                                $singleproducts['image'] = !empty($merchantproduct['image']) ? MERCHANT_PRODUCT_URL.$merchantproduct['image'] : '';
                        ...
                        $sqlcategoryDetail = 'select 0 foodtype, \'Recommended\' food_category 
                                ,null category_img
                                    ...
                                    $categoryDetail = Yii::$app->db->createCommand($sqlcategoryDetail)->queryAll();
										...
										, "location"=>$merchantdetails['location'],'qrType' => $qrType, "logo"=>$merchantlgo, "merchant_mobile"=>$merchantdetails['mobile'], "coverpic"=>$merchantcoverpic
						...
						Yii::trace("============qrcode  responce======".json_encode($payload));
    ...
    try {
		...
		if(!empty($tabel_Det)){
			...
			{
                $payload = ["status"=>'0', 'tableId' => $tabel_Det['ID'] ,"text"=>"Table is already occupied"];
                return $payload;
                exit;
            }
		}
						...
						$valtax = !empty($val['tax']) ? number_format(trim($val['tax']), 2, '.', ',') : 0;
							...
							$userarray['order_id'] =  'SP'.$orderSequence;
							$userarray['txn_id'] = 'SPTX'.$orderSequence; 
							...
							$userarray['ordertype'] = Orders::_ORDER_BY_USER;
							...
							$ordertransmodel->save();
								...
								if(!empty($val['productid'])&&!empty($val['count'])&&!empty($val['price'])){
									...
									$x++; }
											...
											$message = "Hey ".$userdetails['name'].", your order has been placed with Line ID ".$newid."; please wait for further info. Thank you.";
											$image = '';
									//	 Yii::$app->merchant->send_sms($userdetails['mobile'],$message); 
										...
										$coupandetails = Yii::$app->db->createCommand($sqlserviceboyarray)->queryOne();
											...
											if(!empty($serviceboyarray)){
												...
												,'section_name' => $tabel_Det->section['section_name']
											...
											$transaction->commit();
										$payload = array("status"=>'1',"id"=>$orderdetails['ID'],"text"=>"Order Created successfully","order_id" => $orderdetails['order_id']);
							...
							if($result->save()){
								...
								if(!empty($val['productid'])&&!empty($val['count'])&&!empty($val['price'])){
									...
									$x++; }
											...
											$serviceBoyNotiModel->reg_date = date('Y-m-d h:i:s');
											$serviceBoyNotiModel->save();
										$payload = array("status"=>'1',"id"=>$orderdetails['ID'],"text"=>"Order Updated successfully","order_id" => $orderdetails['order_id']);
	...
	public function prepaid($val)
	{
		...
		if(!empty($val['merchantid'])&&!empty($val['table'])&&!empty($val['productid'])&&!empty($val['count'])&&!empty($val['price'])){ 
						...
						$table = trim($val['table']);
							...
							$userarray['merchant_id'] = $merchantid;
							$userarray['tablename'] = 	$table;
							$userarray['order_id'] = 'SP'.Utility::getOrderSequence($merchantid,'DI'); 
	...
	public function orderlist($val){
		...
		$sqlorderlistarray .= ' and orderprocess in (\''.$orderproces_string.'\') ';    
         ...
         if(!empty($val['sdate']) && !empty($val['edate'])){
             $sqlorderlistarray .= ' and date(reg_date) between \''.date('Y-m-d',strtotime($val['sdate'])).'\' 
             and \''.date('Y-m-d',strtotime($val['edate'])).'\' ';
		...
		$orderlistarray = Yii::$app->db->createCommand($sqlorderlistarray)->queryAll();
					...
					foreach($orderlistarray as $orderlist){
						...
						$sql = "select s.section_name from tablename  as tn inner join sections as s on tn.section_id = s.ID 
						where tn.ID  = '".$orderlist['tablename']."'";
						$sectionNameDetails = Yii::$app->db->createCommand($sql)->queryOne();
						...
						$orderarray['section_name'] = @$sectionNameDetails['section_name'];
						...
						/*if(!empty($orderlist['preparetime']) && $orderlist['preparetime'] > '0' && $orderlist['orderprocess'] == '1' && !empty($orderlist['preparedate'])){
							...
							$remainingsec = ((is_numeric($orderlist['preparetime']) ? $orderlist['preparetime'] : 0)  * 60) - $interval;
						...
						$orderarray['rating'] = $feedbackrating;
	...
	public function order($val){
		...
		if(!empty($orderlist)){
			...
			$orderarray['tablename'] = !empty($tableDetails['name']) ? $tableDetails['name'] : ""; 
			...
			if(count($orderproducts) > 0){
				foreach($orderproducts as $orderproduct){
					...
					$productaarray['order'] = $orderproduct['inc'];
					$productaarray['name'] = Utility::product_details($orderproduct['product_id'],'title');
                    $productImage = Utility::product_details($orderproduct['product_id'],'image');
                    $productaarray['image'] = !empty($productImage) ? MERCHANT_PRODUCT_URL.$productImage : '';
...
order by remain_coins desc limit '.$val['userCount'] ;
		...
		if(!empty($val['mobilenumber'])){
			...
			//$resvalidotp['otp'] = '1234';
					...
					if(empty($alreadyid)){
					    ...
					    $sqlUPdate = 'update users set latitude = \''.$val['latitude'].'\',longitude = \''.$val['longitude'].'\'
					      ,name=\''.$val['name'].'\'
					      where ID = \''.$alreadyid['ID'].'\'';
					    $resUpdate = Yii::$app->db->createCommand($sqlUPdate)->execute();
	...
	public function storetypedetails($val)
	{
		...
		,case when (m.logo is null or m.logo = "") then "" else concat(\''.Yii::getAlias('@basePath')."/merchant_images/merchantimages/".'\',m.logo) end logo
		,case when (m.qrlogo is null or m.qrlogo = "") then "" else concat(\''.Yii::getAlias('@basePath')."/merchant_images/merchantimages/".'\',m.qrlogo) end qrlogo
		,case when (m.coverpic is null or m.coverpic = "")  then "" else concat(\''.Yii::getAlias('@basePath')."/merchant_images/merchantimages/".'\',m.coverpic) end coverpic
		...
		left join (select count(ID) orderCount,merchant_id from  orders where orderprocess in (\'4\') group by merchant_id ) C on m.ID =C.merchant_id  
		where m.status = \''.MyConst::TYPE_ACTIVE.'\' 
		and m.storetype in (\''.$room_reservation_types_ids_string.'\') order by C.orderCount desc  ';
	...
	public function upselling($val)
	{
        ...
        if(!empty($merchantid)){
    	 	...
    	 	, 'status' => MyConst::TYPE_ACTIVE,'upselling' => MyConst::TYPE_ACTIVE,'availabilty' => MyConst::TYPE_ACTIVE])
    	 	->asArray()->all();
    	 	...
    	 	if(!empty($upsellingprodcuts)){
    	 	    if(!empty($val['orderedproducts']))
    	 	    {
    	 	         ...
    	 	         and upselling = \''.MyConst::TYPE_ACTIVE.'\' and availabilty = \''.MyConst::TYPE_ACTIVE.'\' and foodtype not in (\''.$foodtypestring.'\')';
		...
		$orderdet =  Yii::$app->db->createCommand($sqlorderdet)->queryOne();
		  ...
		  if(!empty($orderdet))
		  {
			...
			,'user_id' => $orderdet['user_id']
                        ...
                        $stitle = 'Customer buzzed you.';
						$smessage = $orderdet['username'].' at '.$orderdet['table_name']. ' ('. $sectionDet['section_name'].')';
						$simage = '';
						Utility::sendNewFCM($orderdet['push_id'],$stitle,$smessage,$simage,'6',null,$orderdet['ID'],$notificationdet); 
	...
	public function getUserWhilist($val){
		...
		$sql = 'select *,case when (logo is null or logo = "") then "" else concat(\''.Yii::getAlias('@basePath')."/merchant_images/merchantimages/".'\',logo) end logo
			,case when (qrlogo is null or qrlogo = "") then "" else concat(\''.Yii::getAlias('@basePath')."/merchant_images/merchantimages/".'\',qrlogo) end qrlogo
			,case when (coverpic is null or coverpic = "")  then "" else concat(\''.Yii::getAlias('@basePath')."/merchant_images/merchantimages/".'\',coverpic) end coverpic 
			from user_whislist uw inner join users u on u.ID = uw.user_id
			inner join merchant m on m.ID = uw.merchant_id where u.ID = \''.$val['header_user_id'].'\' and uw.status = \'1\' order by uw.ID asc';
		
		$res = Yii::$app->db->createCommand($sql)->queryAll();
		$result = [];
		foreach ($res as $res) {
			$res1 = $res;
			$res1['rating'] = $this->getMerchantRating($res['merchant_id']);
			$result[] = $res1; 
		}

        $res = ArrayHelper::index($result, null, 'storetype');
        $wishlist = array();
			foreach($room_reservation_types_ids as $k => $v){
	...
	public function addPartnerWithUs($val)
	{
		...
		$model->business_type = $val['business_type'];
		$model->business_name = $val['business_name'];
		$model->contact_person = $val['contact_person'];
		$model->mobile_number = $val['mobile_number'];
		$model->email_id = $val['email_id'];
		$model->location_city = $val['location_city'];
		$model->reg_date = date('Y-m-d H:i:s A');
		$model->save();

		$payload = ['status' => '1', 'message' => 'Partner Details Added Successfully'];
	...
	public function getMerchantRating($merchantId){
		...
		return !empty($feedbackrating) ? number_format(($feedbackrating['rating']),1) : '0';
	}
    ...
    public function getUserName($val)
    {
        ...
        if(!empty($userDetails)){
            $payload = [
                ...
                'email' => $userDetails['anniversary_date'],
            ];
        }
        else{
            $payload = [
	...
	{
		$tableDetails = Tablename::findOne($val['tableId']);
		$userDetails = Users::findOne($val['header_user_id']);
		if(!empty($tableDetails)){
			$merchantNotidication = new \app\models\MerchantNotifications;
			$merchantNotidication->merchant_id = $tableDetails['merchant_id'];
			$merchantNotidication->message = $tableDetails['name'].'-'.$tableDetails->section['section_name']. ' is Occupied Without Customers';
			$merchantNotidication->seen = '0';
			$merchantNotidication->created_on = date('Y-m-d H:i:s');
			$merchantNotidication->created_by = $val['header_user_id'];
			$merchantNotidication->save();

			$sqlPilotDetails = 'select s.*,o.order_id as orderId from orders o 
			inner join serviceboy s on o.serviceboy_id = s.ID where o.ID = \''.$tableDetails['current_order_id'].'\'';
			$resPilotDetails = Yii::$app->db->createCommand($sqlPilotDetails)->queryOne();

			if(!empty($resPilotDetails)) {
				$notificationdet = ['type' => 'ALERT_ORDER','tablename' => $tableDetails['name']
				, 'order_id' => $resPilotDetails['orderId']
				,'section_name' => $tableDetails->section['section_name']
			];
				$stitle = 'New Customer from an occupied spot';
				$smessage = $userDetails['name'].' is trying to place a new order at occupied '.$tableDetails['name'].' ('.$tableDetails->section['section_name'].')';
				$simage = '';
				
				Utility::sendNewFCM($resPilotDetails['push_id'],$stitle,$smessage,$simage,'6',null,$tableDetails['current_order_id'],$notificationdet);
			}


			$payload = ['status' => '1', 'text' => 'Informed Staff Successfully'];
		} else {
			$payload = ['status' => '0', 'text' => 'Table Details not found'];
		}
