		...
		$selectedpilot = $arr['pilotid'];
			...
			$model->amount = $arr['amount'];
					...
					//$model['paymenttype'] = $arr['payment_mode'];
							...
							$model->paidstatus = '0';
							$model->paymentby = '1';
							$model->ordertype = Orders::_ORDER_BY_POS;
...
select mf.ID,round(avg(rating)) as rating from merchant_feedback mf
	...
	public function foodShorts()
	{
		$sql = 'select * from food_shorts where status = \'1\' ';
		if(!empty($val['sdate'])) {
			$sql .= ' and date(reg_date) between \''.$val['sdate'].'\'  and \''.$val['edate'].'\' ';
		}
		$sql .= ' order by ID desc';

		$foodshorts = Yii::$app->db->createCommand($sql)->queryAll();
		
		$result = $res = [];

		foreach($foodshorts as $foodshorts) {
			$res['ID'] = $foodshorts['ID'];
			$res['title'] = $foodshorts['title'];
			$res['content'] = $foodshorts['content'];
			$res['reg_date'] = date('d-M-Y h:i A',strtotime($foodshorts['reg_date']));

			$sqlImages = 'select image from food_shorts_images where food_short_id = \''.$foodshorts['ID'].'\'';
			$resImages = Yii::$app->db->createCommand($sqlImages)->queryAll();
			$images = [];
			foreach ($resImages as $image) {
				$images[] = Yii::getAlias('@basePath')."/merchant_images/food_shorts/".$image['image'];
			}
	...
	public function articles($val)
	{
		...
		foreach($articles as $article) {
			...
			$res['image'] = Yii::getAlias('@basePath')."/merchant_images/articles/".$article['image'];
			$res['reg_date'] = date('d-M-Y h:i A',strtotime($article['reg_date']));
			$result[] = $res;
		}
	...
	public function autoCancelledOrders($orderId,$tableId)
	{
        ...
        $model->cancel_reason = 'Auto Cancellation';
		$model->closed_by = Yii::$app->user->identity->merchant_id;
		$model->cancelled_by_name = $merchantDetails['name'];
		$model->cancelled_role_name = 'Auto Cancelled';
        $model->save();

		$tabledet = \app\models\Tablename::findOne($tableId);
		if(!empty($tabledet) && $tabledet['current_order_id'] == $orderId) {
