...
class ServiceboyComponent extends Component{
	...
	public function orderswithstatus($val)
	{
		...
		$date = date('Y-m-d');
					...
					foreach($orderlistarray as $orderlist){
						...
						$orderarray['orderline'] =  $orderlist['orderline']; 
						$orderarray['reorder'] =  $orderlist['reorderprocess']; 
						$orderarray['preparetime'] = !empty($orderlist['preparetime']) ? date('d-M-Y h:i:s A',strtotime($orderlist['preparetime'])) : null;
		...
		$date = date('Y-m-d');
					...
					if(!empty($orderlist)){
						...
						if(!empty($userdetails['push_id'])){	
							$data = ['userName' => $userdetails['name'], 'acceptedUserName' => $serviceboydetails['name']
								,'push_id' => $userdetails['push_id'], 'order_id' => $orderlist['ID']	
							];
	...
	public function serveorder($val){
	    			...
	    			if(!empty($order_details)){
						...
						$userdetails = Users::findOne($order_details['user_id']);
						if(!empty($userdetails['push_id'])){
							$data = ['userName' => $userdetails['name'], 'push_id' => $userdetails['push_id'], 'order_id' => $order_details['ID']];
		...
		if(!empty($orderlist)){
		    if(!empty($preparetime)){
				...
				if(!empty(@$orderlist['user_id'])){
					$data = ['user_id' => $orderlist['user_id'] , 'preparetime' => $preparetime,'order_id' => $orderlist['ID']];
		...
		$date = date('Y-m-d');
					...
					if(!empty($orderlist)){
						...
						$payload = array('status'=>'1','message'=>'Preparation date updated successfully');
					    	if(!empty(@$orderlist['user_id'])){
								$data = ['user_id' => $orderlist['user_id'], 'order_id' =>$orderlist['ID']];
					...
					if(!empty($orderlist)){
						if($orderlist['orderprocess'] != '3'){
                            ...
                            if((!empty($sqlorderpush) && in_array($merchantDetails['cancel_decision'],$decisionCancelStatus) &&  ($resorderpush['reject_count']+1) == $resorderpush['order_sent_count'] && $val['cancelconfirm'] == 1) || $val['cancelconfirm'] == 3){
                                ...
                                $image = '';
								$notificationdet = ['type' => 'RUNNING_ORDER_UPDATE','username' => $userdetails['name']];

                                Utility::sendNewFCM($userdetails['push_id'],$title,$message,$image,null,null,$orderid,$notificationdet);
                                }
                                $table_status = null;
                                $current_order_id = 0;
	...
	public function deliverorder($val)
	{
		...
		$date = date('Y-m-d');
					...
					$result =  Yii::$app->db->createCommand($sqlresult)->execute();
						...
						$userdetails = Users::findOne($orderlist['user_id']);
						if(!empty($userdetails['push_id'])){	
							$data = ['userName' => $userdetails['name'], 'push_id' => $userdetails['push_id'], 'order_id' => $orderid];
						...
						$coinstransactions['merchant_id'] = $orderlist['merchant_id'];
	...
	public function qrcode($val)
	{
		...
		$tabel_Det = Tablename::findOne($val['table']);
						...
						$valtax = !empty($val['tax']) ? number_format(trim($val['tax']), 2, '.', ',') : 0;
							...
							$userarray['ordertype'] = Orders::_ORDER_BY_POS;
									...
									for($i=0;$i<count($productidsarray);$i++){
										...
										if($i == 0){
											...
											if(!empty($userdetails['push_id'])){
												...
												Utility::sendNewFCM($userdetails['push_id'],$title,$message,$image,null,null,$orderdetails['ID'],$notificationdet);
											}
