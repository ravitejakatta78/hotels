...
use app\models\ValetNotifications;
use app\models\MerchantEmployee;
use app\models\AllocateParkingToken;
use app\models\ValetDemoRequests;
use yii\helpers\ArrayHelper;

 date_default_timezone_set("asia/kolkata");

class ValetComponent extends Component{
	...
	public function valetdemorequest($val){
		...
		$alt_mobile_number = $val['alt_mobile_number'];
		$lat = $val['lat'];
		$long = $val['long'];

		echo $checkvaletreg = "SELECT * FROM `valet_demo_requests` WHERE mobile_number='$mobile_number'";
        $reschkvaletreg = Yii::$app->db->createCommand($checkvaletreg)->queryOne();
		
		if($reschkvaletreg==''){

				$valetregmodel = new ValetDemoRequests;
				$valetregmodel->business_name = trim($val['business_name']);;
				$valetregmodel->owner_name = trim($val['owner_name']);
				$valetregmodel->location = trim($val['location']);
				$valetregmodel->city = trim($val['city']);
				$valetregmodel->state = trim($val['state']);
				$valetregmodel->pincode = trim($val['pincode']);
				$valetregmodel->mobile_number = trim($val['mobile_number']);
				$valetregmodel->alt_mobile_number = trim($val['alt_mobile_number']);
				$valetregmodel->lat = trim($val['lat']);
				$valetregmodel->long = trim($val['long']);
				$valetregmodel->reg_date = date('Y-m-d');
				//$valetregmodel->save();
				//$payload=1;
				if($valetregmodel->save()){
					$payload = array("status"=>'1',"text"=>"Requested for demo successfully!!!.");
				}else{
					print_r($result->getErrors());exit;
				}		        
		    }else{
				$payload = array("status"=>'0',"text"=>"Mobile Number already register with us.");
			}
			return $payload;
	}
	...
	public function login($val)
	{
		...
		if(filter_var($mymailid, FILTER_VALIDATE_EMAIL)) {
			$sqlrow = "SELECT * FROM merchant_employee WHERE emp_email = '$mymailid' AND emp_status='1'";
		}else{
			$sqlrow = "SELECT * FROM merchant_employee WHERE emp_phone = '$mymailid' AND emp_status='1'";
		}
		$row = Yii::$app->db->createCommand($sqlrow)->queryOne();
		if(!empty($row['ID'])){
		  	if(password_verify($mypassword,$row['emp_password'])){
				if($row['loginaccess']=='0'){
					$userwherearray['ID'] = $row['ID']; 
					$userarray['loginaccess'] = '1';

					$sqlUpdate = 'update merchant_employee set loginaccess = \''.$userarray['loginaccess'].'\',loginstatus=\'1\' where ID = \''.$userwherearray['ID'].'\'';
					$result = Yii::$app->db->createCommand($sqlUpdate)->execute();
					$payload = array("status" => '1',"text" =>"Successfully Logged In.","usersid" => $row,"merchant_id" => $row['merchant_id']);
				}else {
	...
	public function logout($val)
	{
		if(!empty($val['usersid'])){
			...
			$userdetails = MerchantEmployee::findOne($usersid);
			...
			$loginwherearray['ID'] = $userdetails['ID'];
				...
				$sqlupdate = 'update merchant_employee set loginaccess = \'0\',loginstatus = \'0\',push_id = null where ID = \''.$loginwherearray['ID'].'\' ';
				$result = Yii::$app->db->createCommand($sqlupdate)->execute();
    ...
    public function getparkingtoken($val){
		$merchant_id = $val['merchant_id'];
        $sqltoken = "SELECT * FROM `parking_token` WHERE `merchant_id`='$merchant_id' AND `status`='1'";
        $alltoken = Yii::$app->db->createCommand($sqltoken)->queryAll();
		$payload = array("status"=>'1',"tokens"=>$alltoken);
		return $payload;
    }

	public function gettokendetails($val){
		$id = $val['id'];
		$merchant_id = $val['merchant_id'];
        $sqltoken = "SELECT pt.*, m.storename FROM `parking_token` pt inner join merchant m on pt.merchant_id = m.ID
		WHERE pt.ID='$id' AND pt.merchant_id = '$merchant_id'";
        $token_det = Yii::$app->db->createCommand($sqltoken)->queryOne();
		$payload = array("ID"=>'1',"tokens"=>$token_det);
		return $payload;
    }

	public function checkticket($val){
		$token_number = $val['token_number'];
		$merchant_id = $val['merchant_id'];
		$checkticket = "SELECT * FROM `allocate_parking_token` WHERE token_number='$token_number' AND merchant_id = '$merchant_id'";
        $reschktoken = Yii::$app->db->createCommand($checkticket)->queryOne();
		
		if($reschktoken!=''){
			$payload = array("status"=>'0',"text"=>"Token already assigned. Please select another Token.");
		}else{
			$payload = array("status"=>'1',"text"=>"Success.");
		}
		return $payload;
	}

	public function assigntoken($val){
		$valet_id = $val['valet_id'];
		$merchant_id = $val['merchant_id'];
		$token_number = $val['token_number'];
		...
		$checkticket = "SELECT * FROM `allocate_parking_token` WHERE token_number='$token_number' AND merchant_id = '$merchant_id'";
        $reschktoken = Yii::$app->db->createCommand($checkticket)->queryOne();
		// print_r($reschktoken);die;
		if($reschktoken==''){
				// $ticketarray = array();
				// $ticketarray['valet_id'] = trim($val['valet_id']);
				// $ticketarray['merchant_id'] = trim($val['merchant_id']);
				// $ticketarray['token_number'] = trim($val['token_number']);
				// $ticketarray['ticket_id'] = trim($val['ticket_id']);
				// $ticketarray['recorded_date'] = date('Y-m-d');
				// $ticketarray['ticket_status'] = $ticket_status;
				// //$result = insertQuery($userarray,'feedback');  
				// $result = new \app\models\AllocateParkingToken;
				// $result->attributes = $ticketarray;

				$ticketmodel = new AllocateParkingToken;
				$ticketmodel->valet_id = trim($val['valet_id']);;
				$ticketmodel->merchant_id = trim($val['merchant_id']);
				$ticketmodel->token_number = trim($val['token_number']);
				$ticketmodel->ticket_id = trim($val['ticket_id']);
				$ticketmodel->ticket_status = $ticket_status;
				$ticketmodel->recorded_date = date('Y-m-d');
				//$ticketmodel->save();
				//$payload=1;
				if($ticketmodel->save()){
					$payload = array("status"=>'1',"text"=>"Token Assigned.");
				}else{
					print_r($result->getErrors());exit;
				}		        
		    }else{
				$payload = array("status"=>'0',"text"=>"Token already assigned another valet.");
			}
			return $payload;
	}

	public function showTicket($val){
		$token_number = $val['token_number'];
		$merchant_id = $val['merchant_id'];
		$checkticket = "SELECT * FROM `allocate_parking_token` WHERE token_number='$token_number' AND merchant_id = '$merchant_id'";
        $reschktoken = Yii::$app->db->createCommand($checkticket)->queryOne();
		if(count($reschktoken)==0){
			$sqltoken = "SELECT pt.*, m.storename FROM `parking_token` pt inner join merchant m on pt.merchant_id = m.ID
			WHERE pt.token_number='$token_number'";
			$token_det = Yii::$app->db->createCommand($sqltoken)->queryOne();
			$payload = array("ID"=>'1',"tokens"=>$token_det);
		}else{
			$payload = array("status"=>'0',"text"=>"Valet not assigned. Please get a valet assigned to the Token to continue.");
		}
		return $payload;
	}

	public function updateuserdetails($val){
		$token_number = $val['token_number'];
		$customer_name = $val['customer_name'];
		$mobile_number = $val['mobile_number'];
		$merchant_id = $val['merchant_id'];

		$updatetkt =	"update allocate_parking_token set customer_name = '$customer_name',customer_mobile = '$mobile_number' where token_number = '".$token_number."' AND merchant_id = '$merchant_id'"; 
		$result = Yii::$app->db->createCommand($updatetkt)->execute();
		$payload = array('status'=>'1','message'=>'Ticket Created.');
		return $payload;
	}

	public function updatelocation($val){
		$token_number = $val['token_number'];
		$merchant_id = $val['merchant_id'];
		$vehicle_number = $val['vehicle_number'];
		$parking_pic = $val['parking_pic'];
		$latitude = $val['latitude'];
		$longitude = $val['longitude'];
		$ticket_status = '2';

		$updatetkt =	"update allocate_parking_token set vehicle_number = '$vehicle_number', parking_pic = '$parking_pic', latitude = '$latitude', longitude = '$longitude', ticket_status = '$ticket_status' where token_number = '".$token_number."' AND merchant_id = '$merchant_id'";
		$result = Yii::$app->db->createCommand($updatetkt)->execute();
		$payload = array('status'=>'1','message'=>'Location Updated and vehicle is parked.');
		return $payload;
	}

	public function getuserhistory($val){
		$customer_mobile = $val['customer_mobile'];
		$sqluserdet = "SELECT * FROM `allocate_parking_token` apt left join `merchant` m on apt.merchant_id = m.ID
		left join `merchant_employee` me on me.ID = apt.valet_id
		WHERE apt.customer_mobile='$customer_mobile'";
        $userdet = Yii::$app->db->createCommand($sqluserdet)->queryAll();
		$payload = array("status"=>'1',"user_history"=>$userdet);
		return $payload;
	}

	public function getmycar($val){
		$token_number = $val['token_number'];
		$merchant_id = $val['merchant_id'];
		$get_time = $val['get_time'];
		$ticket_status = '3';

		$updatetkts =	"update allocate_parking_token set get_time = '$get_time', ticket_status = '$ticket_status' where token_number = '".$token_number."' AND merchant_id = '$merchant_id'";
		$result = Yii::$app->db->createCommand($updatetkts)->execute();
		$payload = array('status'=>'1','message'=>'Time Recorded.');
		return $payload;
	}

	public function valetnotification(){
		$token_number = $val['token_number'];
		$get_time = $val['get_time'];

		$notificaitonarary = array();
		$notificaitonarary['merchant_id'] = $orderlist['merchant_id'];
		$notificaitonarary['valet_id'] = $val['header_user_id'];
		$notificaitonarary['order_id'] = $token_number;
		$notificaitonarary['title'] = 'Get My Car';
		$notificaitonarary['message'] = 'Get My Car in '.$get_time.' minutes to the valet';
		$notificaitonarary['seen'] = '0';
						
		$valetNotiModel = new  ValetNotifications;
		$valetNotiModel->attributes = $notificaitonarary;
		$valetNotiModel->reg_date = date('Y-m-d H:i:s');
		$valetNotiModel->mod_date = date('Y-m-d H:i:s');
		$valetNotiModel->save();
	}

	public function releasecar($val){
		$token_number = $val['token_number'];
		$merchant_id = $val['merchant_id'];
		$rating = $val['rating'];
		$to_time = date('h:m');
		...
		$releasesql =	"update allocate_parking_token set to_time = '$to_time', rating='$rating',ticket_status = '$ticket_status' where token_number = '".$token_number."' AND merchant_id = '$merchant_id'";
		$result = Yii::$app->db->createCommand($releasesql)->execute();
		$payload = array('status'=>'1','message'=>'Vehicle Delivered');
		return $payload;
	}

	public function getvalethistory($val){
		$valet_id = $val['valet_id'];
		$sqluserdet = "SELECT * FROM `allocate_parking_token` apt left join `merchant` m on apt.merchant_id = m.ID
		left join `merchant_employee` me on me.ID = apt.valet_id
		WHERE apt.valet_id='$valet_id'";
        $userdet = Yii::$app->db->createCommand($sqluserdet)->queryAll();
		$payload = array("status"=>'1',"valet_history"=>$userdet);
		return $payload;
	}
	...
	public function orderswithstatus($val)
	{
		...
		$date = date('Y-m-d');
					...
					foreach($orderlistarray as $orderlist){
						...
						$orderarray['cancelled_by_otp'] = ( $merchantdetails['cancel_with_otp'] == 1 ) ? true : false ;
	...
	public function order($val)
	{
		...
		if(!empty($orderlist)){
			$tableDetails = Tablename::findOne($orderlist['tablename']);	
			$orderarray = $totalordersarray = array(); 
			$userDetails = !empty($orderlist['user_id']) ? Users::findOne($orderlist['user_id']) : null;
            ...
            $orderarray['ordertypetext'] =  Utility::orderTypeText($orderlist['ordertype']);
			$orderarray['decision_making'] = in_array($merchantdetails['cancel_decision'],$decisionCancelStatus) ? true : false;	
			$orderarray['profilepic'] =  !empty($userDetails) ?  Utility::user_image($userDetails['ID']) : "";
			$orderarray['cancelled_by_otp'] = ( $merchantdetails['cancel_with_otp'] == 1 ) ? true : false ;
					...
					if(!empty($orderlist)){
						if($orderlist['orderprocess'] != '3'){
                            ...
                            if((!empty($sqlorderpush) && in_array($merchantDetails['cancel_decision'],$decisionCancelStatus) &&  ($resorderpush['reject_count']+1) == $resorderpush['order_sent_count'] && $val['cancelconfirm'] == 1) || $val['cancelconfirm'] == 3){
								...
								, closed_by = \''.$val['header_user_id'].'\', cancelled_by_name = \''.$serviceboydetails['name'].'\'
								, cancelled_role_name = \'Pilot\' 
								where ID = \''.$roderwharray['ID'].'\'';
                                $result = Yii::$app->db->createCommand($sqlUpdate)->execute();
                                $userdetails = Users::findOne($orderlist['user_id']);
                                if(!empty($userdetails['push_id'])){
	...
	public function qrcode($val)
	{
		...
		$merchant_details = Merchant::findOne($merchantid);
            ...
            //$latlngdistance = Utility::haversineGreatCircleDistance($latfrom,$lngfrom,$merchant_details['latitude'],$merchant_details['longitude']);

            //if($latlngdistance > $merchant_details['scan_range']){
            //    $payload = array("status"=>'0',"text"=>"Restaurent or  theater distance is too long");
            //    return $payload;
            //    exit;
            //}
    ...
    public function sendotp($val)
    {
        if($val['merchant_id']){
            $merchant = Merchant::findOne($val['merchant_id']);
            if(!empty($merchant)){
                $otp = rand(0000,9999);
                $message = "Hi ".$otp." is OTP for your Registration.";
                \app\helpers\Utility::otp_sms($merchant->mobile,$message,$otp);
                $payload = ['status' => '1', 'otp' => $otp];
            }else{
                $payload = ['status' => '0', 'message' => 'Invalid merchant details'];
            }
