...
class MerchantCoupon extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['merchant_id',   'purpose', 'code', 'validity', 'description', 'type', 'price', 'fromdate', 'todate', 'status', 'minorderamt', 'reg_date'], 'required'],
            [['validity', 'minorderamt'], 'integer'],
            ['maxamt', 'required', 'when' => function($model) {
                return $model->type == 'percent';
            }
                , 'whenClient' => "function (attribute, value) {
                return ($('#merchantcoupon-type').val() == 'percent' || $('#merchantcoupon-type').val() == '');
                }"
            ],
			['code', 'unique', 'on' => 'uniquescenario'],
            [['description', 'type', 'price', 'fromdate', 'todate', 'status', 'maxamt', 'coupon_for'], 'string'],
