...
select distinct(count(created_on)) present_dates,employee_id,1 con_join from employee_attendance where merchant_id = '8' and created_on between '2020-05-01' and '2020-09-14' group by employee_id
        ...
        public function actionReportcategory(){
            ...
            $sqlcategory = "SELECT food_category,p.title,food_type_name,sum(count) orytotal_sale_qty,sum(couponamount) discount,sum(op.price) totalamount FROM orders o 
                        ...
                        WHERE date(o.reg_date) between '".$sdate."' and '".$edate."'
                        and o.merchant_id = '".$merchant_id."' and o.orderprocess = '4' ";
                if(!empty($fc_id)){
                    $sqlcategory .= " and fc.ID = '".$fc_id."'";
                }
                        $sqlcategory .= " group by food_category,p.title,food_type_name";
                       
            $category = Yii::$app->db->createCommand($sqlcategory)->queryAll();
            $categoryArray = array_column($category,'food_category');
            $uniqueCategory = array_unique($categoryArray);
            $categoryValue = array_values($uniqueCategory);
            ...
            //echo "<pre>";print_r($foodcategories);exit;
            //echo $sdate;exit;
            return $this->render('reportcategory',['sdate'=>$sdate,'edate'=>$edate,'categories'=>$category,'foodcategories'=>$foodcategories,
            'fcid'=>$fc_id,'categoryValue'=>$categoryValue]);
        }
        ...
        public function actionSectionWiseSale()
        {
            ...
            $sql = 'select s.section_name,count(o.ID) orderCount,sum(amount) amount,sum(tax) tax,sum(couponamount) couponamount
                    ,sum(tips) tips,sum(discount_number) discount_number,sum(totalamount) totalamount 
                    from orders o 
