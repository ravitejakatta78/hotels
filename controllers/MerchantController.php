...
class MerchantController extends GoController
{
	...
	if ($model->load(Yii::$app->request->post())) {
		...
		if($image){
			...
			$image->saveAs('../../merchant_images/productimages/' . $imagename);
...
else if($tablename == 'role')
{
	...
	$sectionCategory = [];
	foreach ($allcategeries as $category) {
		$sectionCategory[$category['food_section_id']][] = strtoupper($category['food_category']);
	} 
	
	$foodSections = FoodSections::find()->where(['merchant_id' => $merchantId])->asArray()->all();
	
	$merchantPrinters = MerchantPrinters::find()
	...
	if (!empty($food_category)){
		...
		$foodcatgerymodel['category_printer'] = $category_printer; 
	...
	,'site_url' => SITE_URL, 'merchantPrinters' => $merchantPrinters, 'sectionCategory' => $sectionCategory]);
    }
	...
	$sqlcategorytypes = 'select fc.ID,fc.category_printer,fc.food_category,fct.food_type_name,fct.ID fcid,food_section_id,fc.upselling  from food_categeries fc left join food_category_types fct on fc.id = fct.food_cat_id 
	...
	,'foodSections'=>$foodSections,'merchantPrinters' => $merchantPrinters]);		
    }
	...
	|| (trim($foodCategoryUpdate['category_printer']) != trim($_POST['update_category_printer']))
	)
	{
		...
		$foodCategoryUpdate->category_printer = trim($_POST['update_category_printer']);	
  ...
  Yii::$app->getSession()->setFlash('success', [
	...
	public function actionDeletefoodcategorytype()
	{
		extract($_POST);
		$modal = FoodCategoryTypes::findOne($id);
		$modal->delete();
		return "1";
	}
		...
		$image = UploadedFile::getInstance($model, 'image');
			if($image){
				if(!empty($productUpdate['image'])){
					$imagePath =  Yii::getAlias('@basePath').'/merchant_images/productimages/'. $productUpdate['image'];
    ...
    public function actionPilot()
	{
		...
		$empRole = EmployeeRole::find()->where(['merchant_id'=>$merchantId,'ID'=>'2'])->asArray()->all();
		$empRoleIdNameArr = array_column($empRole,'role_name','ID'); 
		$categorytypes = Sections::find()->where(['merchant_id' => $merchantId])
		->asArray()->All();
		$sql = "select count(*) as pilot_buzz,pilot_id  from pilot_buzz
		where date(reg_date) = '".date('Y-m-d')."' and merchant_id = '".$merchantId."'
		GROUP BY pilot_id";
		$pilot_buzz = Yii::$app->db->createCommand($sql)->queryAll();
		//echo "<pre>";print_r($pilot_buzz);exit;
		$pilotBuzzArray = array_column($pilot_buzz,'pilot_buzz','pilot_id');
		//echo "<pre>";print_r($pilotBuzzArray);exit;	
		return $this->render('pilot',['pilotModel'=>$pilotModel,'model'=>$model,'empRoleIdNameArr'=>$empRoleIdNameArr,
		'categorytypes'=>$categorytypes,'pilotBuzzArray'=>$pilotBuzzArray]);
    }
...
if(!empty($_POST['ckcDel'])){
	...
	public function actionGallery()
	{
		...
		if ($model->load(Yii::$app->request->post()) ) {
			...
			if($image){
				$imagename = strtolower(base_convert(time(), 10, 36) . '_' . md5(microtime())).'.'.$image->extension;
				$image->saveAs('../../merchant_images/merchantgallery/' . $imagename);
				$model->image = $imagename;
			}
			if($model->validate()){
				$model->save();
	...
	public function actionDeletegallery()
	{
		...
		if($galleryDet['image']){
				$imagePath =  Yii::getAlias('@basePath').'/merchant_images/merchantgallery/'. $galleryDet['image'];
					if(file_exists($imagePath)){
						unlink($imagePath);	
					}
	...
	$product_popup = $_POST['product_popup'];
		...
		$sqlprevOrderProductDetails = 'select sum(CAST(count AS UNSIGNED)) cnt,product_id,price from order_products where order_id = \''.$order_det->ID.'\'   
		...
		//$prevOrderIdArr = array_column($prevOrderProductDetails,'ID');
					...
					//echo "<pre>";print_r($prevOrderProductsArr);exit;
										...
										$prevOrderProductsPriceArr = (array_column($prevOrderProductDetails,'price','product_id'));
...
//	echo "<pre>";		print_r($orderUpdateArray);exit;        
		...
		for($d=0;$d<count($diff_del_arr);$d++){
			...
			$removeProductInputs = ['merchant_id' => $order_det->merchant_id
			, 'order_id' => $order_det->ID, 'product_id' => $diff_del_arr[$d]
			, 'order_count' => $prevOrderProductsCntArr[$diff_del_arr[$d]]
			, 'price' => $prevOrderProductsPriceArr[$diff_del_arr[$d]]
		];
			Yii::$app->order->removedProducts($removeProductInputs);
		}
		    ...
		    if(in_array($product_popup[$prev],$prevOrderProductsArr)){
		        if($orderUpdateArray[$product_popup[$prev]]  != $prevOrderProductsCntArr[$product_popup[$prev]]){
				 ...
				 if($orderCount > 0){
						...
						$diffCount = $prevOrderProductsCntArr[$product_popup[$prev]] - $orderUpdateArray[$product_popup[$prev]];
							$removeProductInputs = ['merchant_id' => $order_det->merchant_id
							, 'order_id' => $order_det->ID, 'product_id' => $product_popup[$prev]
							, 'order_count' => $diffCount
							, 'price' => $prevOrderProductsPriceArr[$product_popup[$prev]]
						];
							Yii::$app->order->removedProducts($removeProductInputs);
		               }
		               	$insertCondition++;  
...
//			->execute();
	...
	public function actionTableplaceorder()
	{
		...
		$sqlPrevWeekOrders = 'select ID,tablename,orderprocess from orders where date(reg_date) between \''.date('Y-m-d', strtotime('-120 days')).'\' 
	...
	public function actionTableorderstatuschange()
	{
		...
		if(!empty($kdschange)){
		    $this->kdsClearOrder($orderUpdate,$tableUpdate,$id);
			if($kdschange == 1) {
				if(!empty(@$orderUpdate['user_id'])){
					$data = ['user_id' => $orderUpdate['user_id'], 'order_id' =>$orderUpdate['ID']];
					Yii::$app->notification->userFoodPrepared($data); 
				}
			}
		}
	...
	public function kdsClearOrder($orderUpdate,$tableUpdate,$id) 
	{
		$orderUpdate->preparedate = date('Y-m-d H:i:s');
		if(empty($orderUpdate['preparation_time'])) {
			$orderUpdate->preparetime = date('Y-m-d H:i:s');
			$orderUpdate->preparation_time = 20;
		}
		
		$orderUpdate->orderprocess = '1';
	...
	public function actionKdsPreparedOrders() {
		extract($_POST);
		foreach($orderIds as $orderId) {
			$orderUpdate = Orders::findOne($orderId); 
			$tableUpdate = Tablename::findOne($orderUpdate['tablename']);
			
			$this->kdsClearOrder($orderUpdate,$tableUpdate,$orderId);	
			$orderUpdate->save();
		}
		return 1; 
	}
	...
	public function actionApplycouponautocomplete()
	{
		...
		$like = '%'.$query.'%';
		$sql = 'SELECT  code,concat(price,\'-\',type) price, 1 scenario  from merchant_coupon where code like \''.$like.'\' 
...
if($model->load(Yii::$app->request->post())){
	...
	if($logoimage){
					if(!empty($oldLogo)){
					$imagePath =  Yii::getAlias('@basePath').'/merchant_images/merchantimages/'. $oldLogo;
					if(file_exists($imagePath)){
						unlink($imagePath);	
					}
			...
			if($qrlogoimage){
				if(!empty($oldQrLogo)){
					$imagePath =  Yii::getAlias('@basePath').'/merchant_images/merchantimages/'. $oldQrLogo;
					if(file_exists($imagePath)){
						unlink($imagePath);	
					}
				}
			...
			if($coverpicimage){
				if(!empty($oldCoverpic)){
					$imagePath =  Yii::getAlias('@basePath').'/merchant_images/merchantimages/'. $oldCoverpic;
					if(file_exists($imagePath)){
						unlink($imagePath);	
					}
				}
...
if($model->validate()){
	...
	public function actionRemoveimage(){
		...
		if(!empty($removableImage)){
					$imagePath =  Yii::getAlias('@basePath').'/merchant_images/merchantimages/'. $removableImage;
					if(file_exists($imagePath)){
						unlink($imagePath);
						$model->$col = null;
						$model->save();
	...
	public function actionIngredients()
	{
		...
		if ($model->load(Yii::$app->request->post()) ) {
			...
			if($image){
				$imagename = strtolower(base_convert(time(), 10, 36) . '_' . md5(microtime())).'.'.$image->extension;
				$image->saveAs('../../merchant_images/ingredients/' . $imagename);
		...
		$image = UploadedFile::getInstance($model, 'photo');
			if($image){
				if(!empty($ingredientsUpdate['photo'])){
					$imagePath =  Yii::getAlias('@basePath').'/merchant_images/ingredients/'. $ingredientsUpdate['photo'];
					if(file_exists($imagePath)){
						unlink($imagePath);	
					}
				}
...
//print_r($_POST);exit;
	...
	public function actionViewkdsone()
	{
		...
		,fc.food_category,o.reg_date
		,case when food_type_name is not null then concat(title ,\' (\' , food_type_name , \')\') else title end  title_quantity
		,od.ID order_product_id,od.item_deliver_status,o.ordertype
		...
		$tableOrderType = array_column($res,'ordertype','table_order_id');
		$tableIds = array_column($res,'tableid','table_order_id');
		$tabletimeres = array_column($res,'reg_date','table_order_id');
		$orderIds = array_column($res,'table_order_id');
		asort($tabletimeres);
		$tableorderidres = array_column($res,'order_id','table_order_id');
		...
		,'tabletimeres'=>$tabletimeres,'resindex'=>$resindex,'tableorderidres'=>$tableorderidres
		,'res'=>$res,'tableOrderType' => $tableOrderType, 'orderIds' => $orderIds,'tableIds' => $tableIds]);
	}
  public function actionTesting()
  {
	...
	{
				$sqlTableDetails = 'select o.ID,o.orderline,tn.ID tableId,tn.name,o.orderprocess,o.reg_date
		,o.totalamount,current_order_id,o.ordertype,o.serviceboy_id,o.user_id,s.loginstatus from tablename tn 
		inner join orders o on tn.ID = o.tablename 
		left join serviceboy s on s.id = o.serviceboy_id
	...
	//	print_r($tableDetails);exit;
		...
		$merchantDetails = Merchant::findOne(Yii::$app->user->identity->merchant_id);
		//echo "<pre>";print_r($merchantDetails);exit;
	...
	public function actionUpdateorderstatus()
	{
		...
		$userdetails = Users::findOne($orderDet['user_id']);
	    $connection = \Yii::$app->db;	
	    $transaction = $connection->beginTransaction();
    	try {
			if(!empty($tableUpdate)) {
			...
			if (!empty(@$userdetails['push_id'])) {	
				$data = ['userName' => $userdetails['name'], 'push_id' => $userdetails['push_id'], 'order_id' => $orderDet['ID']];
				Yii::$app->notification->userOrderComplete($data);
			}			
...
curl_close($curl);
	...
	public function actionTokenqrdownload()
	{
		extract($_POST);
		$res = ParkingToken::findOne($tableid) ;
		if(empty($res['qr_path'])){
			$qrtext = $this->encrypt($tableid,$userid);
			$sqlQrUpdate = 'update parking_token set qr_path = \''.$qrtext.'\' where merchant_id = \''.Yii::$app->user->identity->merchant_id.'\'
			and ID = \''.$tableid.'\'';
			$resQrUpdate = Yii::$app->db->createCommand($sqlQrUpdate)->execute();
			 $centralurl1 = SITE_URL.'qrdownload.php';
...
// 	print_r($categorytypes);exit;
	...
	public function actionTesttime(){
		Yii::$app->notification->userStartedFoodStarted([1]); 
	...
	if ($model->load(Yii::$app->request->post()) ) {
		...
		if($category_pic){
			...
			$category_pic->saveAs('../../merchant_images/roomcategoryimages/' . $imagename);
		...
		$image = UploadedFile::getInstance($model, 'category_pic');
			if($image){
				if(!empty($roomUpdate['category_pic'])){
					$imagePath =  Yii::getAlias('@basePath').'/merchant_images/roomcategoryimages/'. $roomUpdate['category_pic'];
					if(file_exists($imagePath)){
						unlink($imagePath);	
					}
				}
				...
				$imagename = strtolower(base_convert(time(), 10, 36) . '_' . md5(microtime())).'.'.$image->extension;
				$image->saveAs('../../merchant_images/roomcategoryimages/' . $imagename);
...
//                       echo '<pre>'; print_r($data);exit;
  ...
  Yii::$app->getSession()->setFlash('success', [
    ...
    public function actionNewpos()
	{
		...
		$productAvailablityArr = array_column($productDetails,'availabilty','ID');
		...
		if(!empty($current_order_id) && $current_order_id > 0 )
		{
			...
			if($prevFullSingleOrderDet['orderprocess'] != '3' && $prevFullSingleOrderDet['orderprocess'] != '4'){
				$sqlPrevOrderDetails = 'select o.user_id,op.order_id,op.merchant_id,op.product_id
				,sum(op.count) count,(op.price) price
				,sum(coalesce(op.count,0)*coalesce(op.price,0)) 
			    as totalprice,u.name,u.mobile from 
				orders o inner join order_products op on o.ID = op.order_id 
				left join users u on o.user_id = u.ID 
			    where op.order_id = \''.$current_order_id.'\' and op.count != \'0\' group by 
				o.user_id,op.order_id,op.merchant_id,op.product_id
		...
		, 'prevWeekTableOrderStatus' => $prevWeekTableOrderStatus, 'productAvailablityArr' => $productAvailablityArr
		]);
	}	
	public function actionGetusernamebymobile(){
		$sql = "select * from users where mobile = '".$_POST['mobilenbr']."'";
		$userdetails = Yii::$app->db->createCommand($sql)->queryOne();
	...
	public function actionTitleimages()
	{
		...
		if ($model->load(Yii::$app->request->post()) ) {
			...
			if($title_pic){
				...
				$title_pic->saveAs('../../merchant_images/titleimages/' . $imagename);
		...
		$image = UploadedFile::getInstance($model, 'title_pic');
			if($image){
				if(!empty($oldRoomImage)){
					$imagePath =  Yii::getAlias('@basePath').'/merchant_images/titleimages/'. $oldRoomImage;
					if(file_exists($imagePath)){
						unlink($imagePath);	
					}
				}
				...
				$imagename = strtolower(base_convert(time(), 10, 36) . '_' . md5(microtime())).'.'.$image->extension;
				$image->saveAs('../../merchant_images/titleimages/' . $imagename);
...
$catModel = AllocatedRooms::findOne($resUpdate['room_alocated']);
		...
		public function actionUploadtableexcel()
	{
      ...
      if ($file) {
        $filename = '../../merchant_images/tables/' . $file->name;
	...
	public function actionUploadItems()
	{
		...
		if (Yii::$app->request->isPost) {
          ...
          if ($file) {
                $filename = '../../merchant_images/items/' . $file->name;
                $file->saveAs($filename);
    		    $morefoodsections = 0;
                for($i=2;$i>=0;$i--){
	...
	public function actionAcceptandserveorder()
	{
		...
		if(!empty(@$userdetails['push_id'])){
			$data = ['userName' => $userdetails['name'], 'push_id' => $userdetails['push_id'], 'order_id' => $model['ID']];
			if($_POST['orderstatus'] == '1') {
				if(!empty($model['serviceboy_id'])) {
					$acceptedUser = Serviceboy::findOne($model['serviceboy_id']);
				} else {
					$acceptedUser = Merchant::findOne($model['merchant_id']);
				}
				$data['acceptedUserName'] = !empty($model['serviceboy_id']) ? $acceptedUser['name'] : $acceptedUser['storename'];
				
				Yii::$app->notification->userOrderAccepted($data);	
			} else if($_POST['orderstatus'] == '2') {
				Yii::$app->notification->userServeOrder($data);	
			}
		}
	...
	public function actionSetorderpreparetime()
	{
		...
		if(!empty($model['user_id'])){
			$data = ['user_id' => $model['user_id'] , 'preparetime' => $preptime,'order_id' => $model['ID']];
			Yii::$app->notification->userStartedFoodStarted($data); 
		}
	...
	public function actionCancelreasonpos()
	{
		...
		$model->cancelled_by_name = $merchantDetails['name'];
		$model->cancelled_role_name = 'Merchant';
        $model->save();
	...
	public function actionPilotbuzz()
	{
		...
		$callPilot = new PilotBuzz;
		$callPilot->pilot_id = $empId;
		$callPilot->merchant_id = Yii::$app->user->identity->merchant_id;
		$callPilot->reg_date = date('Y-m-d H:i:s');
		$callPilot->save();
		return 1;
	}
	public function actionUserbuzz()
	{
		extract($_POST);
		$details = Users::findOne($userId);
		$merchantDetails = Merchant::findOne(Yii::$app->user->identity->merchant_id);
		$orderDet = Orders::findOne($orderId);
		$notificationdet = ['type' => 'USER_BUZZ','username' => $details['name']
		,'user_id' => $details['ID'],'order_id' => $orderDet['order_id']];
		$stitle = 'Buzz from counter';
		$smessage = 'Your Order is Ready';
		$simage = '';
        $extraNotificationData = [
            'buzz_by' => Yii::$app->user->identity->emp_name,
            'token_no' => $orderLine,
            'rest_name' => $merchantDetails['storename']
        ];
		Utility::sendNewFCM($details['push_id'],$stitle,$smessage,$simage,'6',null,null,$notificationdet,$extraNotificationData);
	}
    ...
    public function actionAddPrinters()
    {
        ...
        $model = new MerchantPrinters;
//        $model->scenario = 'insertprinter';
//        if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
//            Yii::$app->response->format = Response::FORMAT_JSON;
//            return ActiveForm::validate($model);
//        }
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            if(empty($merchantPrinters)){
                $model->default_printer = 1;
            } else {
                $model->default_printer = 2;
            }
            ...
            if($model->validate()){
                ...
                return $this->redirect('add-printers');
            }
            else
            {
                echo "<pre>";print_r($model->getErrors());exit;
        ...
        return $this->render("addprinters",['merchantPrinters' => $merchantPrinters,'model' => $model]);
    }

    public function actionEditprinterpopup()
    {
        extract($_POST);
        $printerModel = MerchantPrinters::findOne($id);
        return $this->renderAjax('editprinterpopup', ['model' => $printerModel,'id'=>$id]);
    }
    public function actionEditprinter()
    {
        ...
        $tableNameArr = Yii::$app->request->post('MerchantPrinters');
        $tableNameUpdate = MerchantPrinters::findOne($_POST['MerchantPrinters']['ID']);

        $tableNameUpdate->attributes = \Yii::$app->request->post('MerchantPrinters');

        if($tableNameUpdate->validate()){
            $tableNameUpdate->save();
            Yii::$app->getSession()->setFlash('success', [
                'title' => 'Spot',
                'text' => 'Printer Settings Edited Successfully',
                'type' => 'success',
                'timer' => 3000,
                'showConfirmButton' => false
            ]);
    ...
    public function actionValetlist()
	{
		$model = new MerchantEmployee;
		$model->scenario = 'insertemail';
		if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
		...
		$sqlvalet = "SELECT me.* FROM `merchant_employee` me LEFT JOIN `employee_role` er ON me.emp_role = er.ID WHERE me.merchant_id=".$merchantId." AND er.role_name='Valet'";
		$empModel = Yii::$app->db->createCommand($sqlvalet)->queryAll();
		$connection = \Yii::$app->db;	
		$transaction = $connection->beginTransaction();
		try {		
			if ($model->load(Yii::$app->request->post()) ) {
				...
				if ( $model->validate() ) {
					...
					,[
						'title' => 'Valet',
						'text' => 'Valet Created Successfully',
						'type' => 'success',
						'timer' => 3000,
						'showConfirmButton' => false
					]);
					if(@$_POST['pilotrender']){
						...
						return $this->redirect('valetlist');
					}
...
// 			print_r($categorytypes);exit;
        ...
        return $this->render('valetlist',['model'=>$model,'empModel'=>$empModel,'empRoleIdNameArr'=>$empRoleIdNameArr
            ,'categorytypes'=>$categorytypes,'id' => $merchantId]);
	}

    public function actionEditvaletpopup()
    {
        extract($_POST);
        $empModel = MerchantEmployee::findOne($id);
        $serviceBoy =  Serviceboy::find()
        ...
        return $this->renderAjax('editvaletpopup', ['model' => $empModel, 'id'=>$id
            ,'pilotTableId' => $pilotTableId]);

    }
    public function actionUpdatevalet()
    {
        $model = new MerchantEmployee;
        $model->scenario = 'updateemail';
        if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
        ...
        if($employeeUpdate->validate()){
            ...
            Yii::$app->getSession()->setFlash('success', [
                'title' => 'Valet',
                'text' => 'Valet Details Updated Successfully',
                'type' => 'success',
                'timer' => 3000,
                'showConfirmButton' => false
            ]);
    ...
    public function actionDeletevalet(){
        extract($_POST);
        $MerchantEmployeedetails = MerchantEmployee::findOne($id);
        $MerchantEmployeedetails->emp_status = '17';
        $MerchantEmployeedetails->save();
    ...
    public function actionParkingtoken(){
        //echo SITE_URL;die;

        extract($_POST);

        $model = new ParkingToken;
        $model->scenario = 'inserttoken';
        if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
            Yii::$app->response->format = Response::FORMAT_JSON;
            return ActiveForm::validate($model);
        }
        ...
        try {
            ...
            if ($model->load(Yii::$app->request->post()) ) {
                ...
                $ParkingToken = Yii::$app->request->post('ParkingToken');

                $model->merchant_id = (string)$merchantId;
                $model->token_number = $ParkingToken['token_number'];
                $model->qr_path = NULL;
                $model->download_qr = '0' ;
                $model->multi_qr = '0' ;
                $model->status = '1' ;
                ...
                if(@$_POST['pilotrender']){
                    ...
                    return $this->redirect('parkingtoken');
                }
        ...
        return $this->render('parkingtoken',['model'=>$model,'tokenModel'=>$tokenModel,'merchantdetails'=>$merchantdetails]);
    }

    public function createParkingToken($tableid,$userid,$tablename){
        $res = ParkingToken::findOne($tableid) ;
        if(empty($res['qr_path'])){
            $qrtext = $this->encrypt($tableid,$userid);
            $sqlQrUpdate = 'update parking_token set qr_path = \''.$qrtext.'\' where merchant_id = \''.Yii::$app->user->identity->merchant_id.'\'
    ...
    public function actionShowtoken(){
        ...
        return $this->render('viewtoken',['qrlogo'=>$qrlogo,'qrpath'=>$qrpath,'tokennumber'=>$tokennumber]);
    }

    public function actionChangeDefaultPrinter()
    {
	    extract($_POST);
	    $printerDet = MerchantPrinters::findOne($id);
        $status = ($printerDet->default_printer == 1) ? 2 : 1;

        if ($status == 1) {
            $oldDefaultPrinter = MerchantPrinters::find()
                ->where(['default_printer' => 1, 'merchant_id' => $printerDet['merchant_id']])
                ->asArray()->One();
            if (!empty($oldDefaultPrinter)) {
                $oldDefaultPrinterChange = MerchantPrinters::findOne($oldDefaultPrinter['ID']);
                $oldDefaultPrinterChange->default_printer = 2;
                $oldDefaultPrinterChange->save();
            }
            $printerDet->default_printer = $status;
            $printerDet->save();
        }
    ...
    public function actionQrone(){
        extract($_POST);

        return $this->renderpartial('qrone',['qrlogo'=>$qrlogo,'qrpath'=>$qrpath,'tablename'=>$tablename]);
    }
...
// Function to create image which contains string.
        ...
        $image = imagecreatefromstring(file_get_contents('../../layout.png'));

        imagestring($image, 5, 140, 30,  "GeeksforGeeks", $text_color);
        $image = imagecreatefromstring(file_get_contents('../../layout.png'));
        imageinterlace($image, false);

        $QR = imagecreatefromjpeg('http://localhost/tutors/web/img/qrimg/qrcode.jpg');
        imagecopyresampled($image,$QR,100,  20,10,50,100,100,50,50);


        header("Content-Type: image/png");
        header("Content-Disposition: Attachment; filename=newimg.png");
	...
	public function actionMultikot(){
		return $this->renderPartial('multikot', ['order_id' => $_POST['id']]);
	}
}