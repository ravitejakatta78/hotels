...
$addedCouponDetails = [];
if(!empty($prevFullSingleOrderDet['coupon'])){
  $addedCouponDetails = MerchantCoupon::find()->where(['code' => $prevFullSingleOrderDet['coupon']])->asArray()->one();
}

$pilotDetails = [];
if(!empty($prevFullSingleOrderDet['serviceboy_id'])){
  $pilotDetails = Serviceboy::findOne($prevFullSingleOrderDet['serviceboy_id']);
}
...
$prevOrderCountArray = array_column($prevOrderDetails,'count','product_id');
        ...
        <div class="col-md-8 pl-0">
          <div class="type-details">
            ...
            <div class="type ordertype <?php if ($tableName == 'PARCEL') { ?> active <?php } ?>" onclick="changetable('PARCEL','','')">
              <i class="fa fa-picture-o"></i> Parcels
            </div>
          ...
          <div class="row">
            <div class="col-md-6 text-center pr-0">
                <div class="pilot" class="btn btn-info btn-lg" >
                    <span data-toggle="modal" <?php if ($tableName != 'PARCEL') { ?> data-target="#myModalPilot" <?php } ?>>Pilot Assign 
                      <span class="ml-2"><i class="fa fa-edit"></i></span>
                    </span>
                    <?php if(!empty($prevFullSingleOrderDet['serviceboy_id'])){ ?>
                      <span class="ml-2" onclick="buzzPilot('<?= $prevFullSingleOrderDet['serviceboy_id'];?>','<?= $pilotDetails[ 'loginstatus']; ?>')">
            ...
            <div class="col-md-6 text-center pl-0">
                <div class="custmer" >
                    <span data-toggle="modal" data-target="#myModalCustomer">Customer 
                        <span class="ml-2" ><i class="fa fa-edit"></i></span>
                    </span>
                    <?php if($merchant_det['reverse_buzz'] == '1' && isset($prevFullSingleOrderDet['user_id']) ) {
                      if($prevFullSingleOrderDet['user_id'] != null && $prevFullSingleOrderDet['ordertype'] == '1') {
                      ?>
                    <span class="ml-2" onclick="buzzUser('<?= $prevFullSingleOrderDet['user_id'];?>','<?= $prevFullSingleOrderDet['ID'] ;?>','<?= $prevFullSingleOrderDet['orderline'] ;?>')"><i class="fa fa-bell"></i></span>
                    <?php } } ?>
                </div>
            </div>
          </div>
                ...
                <table class="table table-striped table-bordered dataTable no-footer mt-0" id="itemtable" style="margin-top: 0px !important">
                  <thead>
                    <tr>
                      ...
                      ?>
                        <th onclick="cancelreject(<?= $prevFullSingleOrderDet['ID']; ?>,<?= $prevFullSingleOrderDet['orderprocess']; ?>)" style="cursor:pointer" title="Cancel Order"><i class="fa fa-times" ></i></th>
                      <?php } else { ?>
                        <th></th>
                      <?php } ?>
              ...
              <div class="col-md-12">
                <div class="row">
                  <div class="ttl-payble" onclick="saveorder()" style="cursor:pointer">
                    <?php echo !empty(@$prevFullSingleOrderDet['orderprocess']) ? 'Update Order' : 'Place Order'; ?> :₹ <span id="ttl_payble"> <?php echo $ttl_amt =  round(@$prevFullSingleOrderDet['totalamount'], 2) ?? 0; ?></span>
                  </div>
                  <div class="buts mt-2 text-center col-md-12">
                    ...
                    <?php if($merchant_det['multi_kot_config'] == '1') { ?>
                    <button class="btn btn-Info mb-2 fnt" onclick="return multikot('<?= $prevFullSingleOrderDet['ID'] ?? ''; ?>');">Multi KOT</button>
                    <?php } ?>
            ...
            ?>
                  <div class="col-lg-2 col-md-2 pl-1 pr-1 filtersearch">
                    <div class="nd_options_section nd_options_position_relative mb-2 min-hgt" onclick="foodquantitypopup('<?php echo $value[$i]['modified_title']; ?>')">
                      ...
                      <img class="nd_options_section" alt="" src="<?= Yii::getAlias('@basePath').'/merchant_images/productimages/'. $imgArr[$value[$i]['modified_title']]; ?>">
...
<!-- Modal -->
<div id="myModalPilot" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog">
    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-9">
            <input type="text" id="usermobile" autocomplete="off" class="form-control" max="10" value="<?= $userDet['mobile'] ?? ''; ?>" onchange="giveusername()"
                    <?php if(!empty($prevFullSingleOrderDet['ordertype']) && (@$prevFullSingleOrderDet['ordertype'] == '1' || @$prevFullSingleOrderDet['ordertype'] == '3') ) { echo 'readonly'; } ?> >
          </div>
        </div>
...
<!-- Modal -->
<div id="myModalCoupon" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog">
    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-footer">
      <?php if(!empty($prevFullSingleOrderDet['coupon'])) { ?>
        <button type="button" class="btn btn-default" onclick="removeCoupon()" data-dismiss="modal">Remove Coupon</button>
      
      <?php } else { ?>
        <button type="button" class="btn btn-default" id="merchant_coupon_save" data-dismiss="modal">Apply</button>
      <?php } ?>
...
<script>
  ...
  function getFoodTypeProducts(foodtype, foodsecvalue) {
    ...
    var baseUrl = '<?= Yii::getAlias('@basePath')."/merchant_images/productimages/"  ?>';
  ...
  var foodquantitypopup = (function() {
      return function(total_modified_name) {
          ...
          var productAvailablityJson = '<?= json_encode($productAvailablityArr); ?>';
          var productAvailablityArr = JSON.parse(productAvailablityJson);
          ...
          for (var i = 0; i < (quantity_arr.length); i++) {
            ...
            if (quantity_arr.length >= 2) {
              $("#myModal").modal('show');
                ...
                if(productAvailablityArr[quantity_arr[0]['ID']] == '0') {
                    swal(
                      'Warning!',
                      'Not Available',
                      'warning'
  ...
  function calCpnAddSub()
  {

    var addedCouponDetailsJson = '<?= json_encode($addedCouponDetails); ?>';
    var addedCouponDetails = JSON.parse(addedCouponDetailsJson);
    
    if(addedCouponDetails['price'] != undefined ){
      

      swal('Warning!',
        'Please remove the coupon and apply it after the order is updated finally',
        'warning')
        .then((value) => {
          refreshorder();
      });
    ...
    if(addedCouponDetails['price'] != undefined ){
      const items = {value: addedCouponDetails['price']+'-'+addedCouponDetails['type'],scenario : 2 };
      displayResult(items);
    } */
  ...
  function saveorder() {
    ...
    if (isNaN(ttl_sub_amt)) {
    ...
    else if (cancel_with_otp_val == 1  && parseFloat(prevAmount) != parseFloat(ttl_sub_amt) 
    ) {
      var prodrowid;
      var itemIdArr = [];
      var itemQtyArr = [];
      $('#itemtable tr').each(function() {
        if (this.id != '') {
            prodrowid = this.id.split("_");
            itemIdArr.push($("#item_id_"+prodrowid[1]).val());
            itemQtyArr.push($("#qtyind_"+prodrowid[1]).val());
          }
        
      });  
      var result =  itemQtyArr.reduce(function(result, field, index) {
          result[itemIdArr[index]] = field;
          return result;
      }, {});
      
      var prevOrderCountJson = '<?= json_encode($prevOrderCountArray); ?>';
      var prevOrderCountArray = JSON.parse(prevOrderCountJson);
      var showPopup = false;
      if(Object.keys(prevOrderCountArray).length > 0) {
        $.each(prevOrderCountArray, function( k, v ) {
            if( result[k] < v || result[k] === undefined ){
  ...
  function multikot(id) {
    ...
    var form = document.createElement('form');
    form.setAttribute('method', 'post');
    form.setAttribute('action', 'multikot');
    form.setAttribute('target', '_blank');
  ...
  function giveusername(){
    var mobilenbr = $("#usermobile").val();
    if(mobilenbr.length != 10) {
      alert("Please Provide Valid Mobile Number");
      return false;
    }
    var request = $.ajax({
      url : "getusernamebymobile",
      type : "POST",
      data : {mobilenbr:mobilenbr},
    }).done(function(msg) {
      //alert(msg);
      /*var res = JSON.parse(msg);
      alert(res['name']);*/
  ...
  function buzzPilot(empId,loginStatus){
    ...
    swal(
          'Success!',
          'Pilot Buzzed Successfully',
          'success'
    );
	});
}

function buzzUser(userId,orderId,orderLine){
	var request = $.ajax({
		url: "userbuzz",
		type: "POST",
		data: {userId : userId,orderId:orderId,orderLine:orderLine},
	}).done(function(msg) {
    swal(
          'Success!',
          'Customer Buzzed Successfully',
          'success'
    );
	});
}
