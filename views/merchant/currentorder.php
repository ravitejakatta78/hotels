...
$parcelCount = (array_values(array_filter(array_column($parcelDetails,'orderprocess')))); 		?>
		...
		<?php if(in_array($merchantdetails['cancel_decision'],$decisionCancelStatus)) { ?>
				...
				<?php } ?>
				<?php 
				if($tableDetails[$i]['serviceboy_id'] != null) { ?>
				<button class="placeorder btn" onclick="buzzPilot('<?= $tableDetails[$i]['serviceboy_id'];?>','<?= $tableDetails[$i][ 'loginstatus']; ?>')">Pilot buzz</button>
				<?php } ?>
				<?php if($merchantDetails['reverse_buzz'] == '1') { 
				if($tableDetails[$i]['user_id'] != null && $tableDetails[$i]['ordertype'] == '1') { ?>
				<button class="placeorder btn" onclick="buzzUser('<?= $tableDetails[$i]['user_id'];?>','<?= $tableDetails[$i]['ID'] ;?>','<?= $tableDetails[$i]['orderline'] ;?>')">Customer buzz</button>
				<?php } ?>
				<?php } ?>
				</div>
			</div>
